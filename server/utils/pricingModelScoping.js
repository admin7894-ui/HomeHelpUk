/**
 * Pricing Model Scoping Engine for HomeHelpUK
...
function getApplicablePricingModels(serviceId, categoryName) {
  // 1. Service-level precision override
...
function isPricingModelAllowed(serviceId, categoryName, pricingModelId, enabledModels = []) {
  if (pricingModelId === 'multi') {
    ...
    return enabledModels.every(em => allowed.some(m => m.id === em));
  }
  const allowed = getApplicablePricingModels(serviceId, categoryName);
...
module.exports = {
  PRICING_MODELS,
  CATEGORY_ALLOWED_MODELS,
  SERVICE_ALLOWED_MODELS,
  getApplicablePricingModels,
  isPricingModelAllowed
};
