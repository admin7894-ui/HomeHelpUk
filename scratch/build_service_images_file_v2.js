...
const domainCatalog = JSON.parse(fs.readFileSync(path.join(__dirname, 'domain_catalog_generated.json'), 'utf-8'));

let fileContent = `/**
 * Complete Service Media Catalog & High-Definition Image Resolver for HomeHelpUK
 * 100% Domain-Isolated & Unique Image Mappings for all 76 Services
 * Includes Animated GIF Previews and dedicated 4-photo galleries for every service
 */

export const SERVICE_MEDIA_CATALOG = ${JSON.stringify(domainCatalog, null, 2)};
...
export const getServicePreviewGif = (serviceId) => {
  const serviceData = SERVICE_MEDIA_CATALOG[serviceId];
  if (serviceData && serviceData.previewGif) {
    return resolveImageSource(serviceData.previewGif);
  }
  return null;
};
`;
...
console.log('Successfully wrote mobile/src/utils/serviceImages.js with domain-isolated imagery & GIF previews');
